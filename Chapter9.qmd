...
arthritis <- arthritis |>
  mutate(Improvement = factor(Improvement, levels=c("Marked","Some","None"),ordered=T),
         Treatment = factor(Treatment,levels=c("Placebo","Active"),ordered=T))
...
mod_po <- polr(Improvement~Treatment+Sex,data=arthritis)
...
```{r}
library(emmeans)
emm_po <- emmeans(mod_po,~Improvement|Treatment*Sex,mode="prob")
emm_po
pairs(emm_po,type="response")

emm_sex <- emmeans(mod_po,~Sex)
pairs(emm_sex,type="response")
```
