...
format:
  revealjs:
    theme: clean.scss
    scrollable: true
    footer: "Chapter 6- [Back to home](https://madfudolig.quarto.pub/eab770slides/)"
    code-block-bg: true
...
-   Recall: 2x2, and sxr tables all use Mantel-Haenszel Statistics
    -   Able to account for ordinality unlike Pearson Chi-Square tests
    -   Less stringent sample size requirements for ordinal variables
...
tables hospital*trt*severity / cmh scores=ridit; # <2>
run;
```
...
dat_cc <- xtabs(Count~Treatment+Severity,data=operate)
dat_cc
library(vcdExtra)
...
Both variables are ordinal, hence we can test for correlation using $Q_{CSMH}$. Using modridit scores, we get $Q_{CSMH}=6.64$, p=0.0100. Hence, we have sufficient evidence that the proportion of gastric tissue removed is associated with symptom severity in patients.
:::
...
weight count;
tables area*location*size*usage / cmh scores=ridit;
;
run;
```
...
```{r}
dat_cc <- xtabs(Count~CarSize+Usage+Area+Location,data=harness)
CMHtest(dat_cc,
        rscores="midrank",
        cscores="midrank")
```
...
Both variables are ordinal. We can use the extended correlation statistic $Q_{CSMH}$. Using ridit/rank-based scores, $Q_{CSMH}$ =6.44, p=0.01. There is sufficient evidence to conclude that car size and shoulder harness usage are correlated after controlling for geographical area and location.

:::

# Repeated Measures

## Repeated Measures: Application 

Repeated measures are commonly encountered in longitudinal studies (e.g. cohort studies). 

::: callout-note
Mantel-Haenszel statistics can be calculated to determine whether there is an association between a repeated measurement factor (usually time) and a response variable adjusting for an effect of subject. Controlling for subject adjusts for the variability between subjects, which focuses the analysis to within-subject.
:::
...
Mantel-Haenszel statistics can be used to test the hypothesis of no association between time and response under the assumption that marginal totals per subject in response and time is fixed.

::: callout-note
The null hypothesis for Mantel-Haenszel tests for repetaed measures is $H_0$: The response variable is distributed at random with respect to the time points.This null hypothesis is referred to as the hypothesis of interchangeability or marginal homogeneity.
...
Please refer to `pump.csv` on Canvas.

```{r}
library(tidyverse)

pump <- read.csv("pump.csv") |> 
...
```{r}
dat_cc<- xtabs(~Time+Response+Subject,data=pump)
...
In `R`, we can create a repeated measures table and apply the McNemar's test.

```{r}
dat2 <- pump |> 
  pivot_wider(names_from=Time,values_from=Response) 
dat_cc2 <- xtabs(~before+after,data=dat2)

mcnemar.test(dat_cc2,correct=F)
```
...
Please refer to `drug.csv` uploaded on Canvas.

```{r}
drug <- read.csv("drug.csv") |> 
  mutate(Patient=as.factor(Patient),
         Outcome=factor(Outcome,levels=c("U","F")))
glimpse(drug)
```
...
```{r}
dat_cc <- xtabs(~Drug+Outcome+Patient,data=drug)
...
The most appropriate MH statistic to examine is the mean score statistic, specifically the row means.
