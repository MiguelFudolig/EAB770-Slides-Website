...
For large $n$, both the MH chi-square and the Pearson chi-square approach a chi-square distribution with $1$ degree of freedom ($\chi^2_1$). The relationship can also be approximated by the following relation:
...
The null hypothesis of no association states that there is no association between row and column variables. 
- Assuming this is true and assumptions are satisfied, $Q$ and $Q_P$ follow a $\chi^2_1$ distribution for $2\times2$ tables.
...
-   The data can be analyzed through SAS or mathematically through the hypergeometric distribution
...
-   Miettinen-Nurminen **0.2460-0.5427**
:::
...
::: columns
::: {.column style="width:50%; font-size: 95%"}
