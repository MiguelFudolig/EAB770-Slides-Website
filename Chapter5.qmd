...
-   Use column 2 (Hours = 1) as cutoff. The use of the chi-square approximation is justified!
...
The Goodman Kruskal's Gamma measures association for two ordinal variables based on the number of discordant and concordant pairs. This ignores any ties that happen in the data set.


### $D$

Somers’ Delta ($D$) is the ratio of concordant and discordant pairs divided by the pairs from either row/column variable.

::: callout-note
Because the denominator is conditional on the margin, $D$ is asymmetric. 
:::

### $\tau_C$
Stuart’s $\tau_C$ includes an adjustment for table size  to a correction for ties. This is recommended when two ordinal variables under consideration have different numbers of values.
...
## Measures of Association: `SAS`

The `measures` option in the `tables` statement produces the different measures of association for the contingency tables. When the sample size is large enough, 95% confidence intervals can be calculated for these measures using a normal approximation. These confidence intervals can be produced using the `cl` option.

$$
measure \pm 1.96*ASE
$$

where ASE is the approximate standard error reported in the `measures` output from the `freq` procedure.

## Measures of Association in `R`

The `DescTools` package includes the functions that measures of association for contingency tables.

- `SomersDelta(direction=c("row","column"))`: Somers' Delta
- `StuartTauC()`: Stuart's $\tau_c$
- `KendallTauA()`: Kendall's $\tau_a$
- `GoodmanKruskalGamma()`: Goodman and Kruskal's Gamma
...
- Pearson correlation (SAS only)
- Spearman Correlation (SAS only)
...
```{r}
library(DescTools)
GoodmanKruskalGamma(dat_cc,conf.level=0.95)
KendallTauB(dat_cc,conf.level=0.95)
StuartTauC(dat_cc,conf.level = 0.95)
SomersDelta(dat_cc,conf.level = 0.95)
```
...
KendallTauB(dat_cc,conf.level=0.95)
StuartTauC(dat_cc,conf.level = 0.95)
SomersDelta(dat_cc,conf.level = 0.95,direction="column")
```
...
Goodman Kruskal Lambda measures the probable improvement in predicting the column variable given knowledge of the row variable.
...
## Nominal Variables: `SAS`

Both the Goodman Kruskal Lambda and uncertainty measures are included in the `measures` output.
...
The `Lambda()` and `UncertCoef()` functions in DescTools calculate the Goodman Kruskal Lambda and Uncertainty measures, respectively. 

::: callout-note
Because the measures could be symmetric or asymmetric, both functions can specify the direction as "symmetric","row", or "column".

```{r}
#| eval: false

Lambda(contingencytable, direction=c("symmetric","row","column"),conf.level = 0.95)
UncertCoef(contingencytable, direction=c("symmetric","row","column"),conf.level = 0.95)
...
Lambda(dat_cc, direction=c("symmetric"),conf.level = 0.95)
UncertCoef(dat_cc, direction=c("symmetric"),conf.level = 0.95)
```
...
The symmetric $\lambda$ is estimated to be 0.11 (95% CI: 0.09,0.13). The estimated proportion of uncertainty in party affiliation explained by the neighborhood is estimated to be 0.04 (0.03, 0.05).
:::
...
:::callout-note
The biggest error is observer error, when observers do not agree about the measurement. How do we account for observer error, especially for multiple observer measurements?
:::
...
The Bowker's test tests whether the contingency table is symmetric about the diagonals.The test works for square tables.

::: callout-note
The test is equivalent to the McNemar’s test when the table is $2\times 2$.
:::
...
- The `AGREE` option in the tables statement generates the kappa statistics and Bowker’s symmetry test.
  ...
  -  Not specifying `(WT=FC)` will produce the Cichetti-Allison criterion instead.
...
## Observer Agreement: `R`

::: panel-tabset

### $\kappa$

The `Kappa()` function in the `vcd` package calculates the Cohen's kappa statistic.

::: callout-tip
We can also specify which weights we use to calculate the weighted kappa statistic. By default, the weighting method used is the Cichetti-Allison method, also referred to as "Equal-Spacing". To use the Fleiss-Cohen weighting method, we need to specify "Fleiss-Cohen".

```{r}
#| eval: false


K_CA <-Kappa(contingencytable,weight="Equal-Spacing")
K_CA
confint(K_CA)

K_FC <- Kappa(contingencytable,weight="Fleiss-Cohen")
K_FC
confint(K_FC)
```

To perform Bowker's Test, we can use the `mcnemar.test()` function in `R`, similar to how we can use `mcnemar.test()` to perform the McNemar's test.
:::

### Agreement Plot

Agreement plots can be used to visualize observer agreements. These can be generated using the `agreementplot()` function in the `vcd` package in `R`.
...
The data comes from a study concerning the diagnostic classification of multiple sclerosis patients. Patients from Winnipeg and New Orleans were classified into one of four diagnostic classes by both a Winnipeg neurologist and a New Orleans neurologist.
  ...
  - Calculate the estimate and 95% confidence interval for the simple and Fleiss-Cohen weighted Kappa statistic.
...
```{r}
dat <- expand.grid(NO=1:4,
                   WN=1:4)

dat$count <- c(38,33,10,3,5,11,14,7,0,3,5,3,1,0,6,10)

dat_cc <- xtabs(count~NO+WN,data=dat)
dat_cc
```
...
```{r}
agreementplot(dat_cc)

K_FC <- Kappa(dat_cc,weights="Fleiss-Cohen")
...
Weighted $\kappa$: 0.52 (0.41,0.64)

Since the weighted $\kappa$ is higher than the simple $\kappa$, we can infer that there were more disagreements closer to the diagonal. There is moderate agreement between the two neurologists based on the weighted $\kappa$.
:::
...
Please refer to `radiology.csv`
```{r}
radio <- read.csv("radiology.csv") |> 
  mutate(across(c("Rater1","Rater2"),~as.factor(.x)))
glimpse(radio)
```
...
- Calculate the simple and weighted (Cichetti-Allison and Fleiss-Cohen) kappa statistics and the corresponding 95% confidence intervals.
- Compare the asymptotic and exact p-values in testing whether there is an agreement between the two radiologists.
...
```{r}
dat_cc <- xtabs(Count~Rater1+Rater2,data=radio)
dat_cc

K_CA <-Kappa(dat_cc,weight="Equal-Spacing")
K_CA
confint(K_CA)

K_FC <- Kappa(dat_cc,weight="Fleiss-Cohen")
K_FC
confint(K_FC)
```
...
Cichetti-Allison Weighted $\kappa$: 0.57 (0.44,0.70)
Fleiss-Cohen Weighted $\kappa$: 0.67 (0.54,0.80)
...
The Jonckheere-Terpstra test is a nonparametric rank-based test that is based on sums of Mann-Whitney test statistics. This test detects whether there are differences in the trend of group effects.

  
:::callout-note
The test performs the hypothesis test that answers the question "Are the scores strictly increasing or decreasing?"

The null hypothesis $H_0$ assumes that the distribution of ordered responses is the same for all groups. 

For one-sided tests, this tests detect whether $d_1 \leq d_2 \leq... d_s$ where $d_i$ corresponds to the $i^{th}$ group effect.

For two-sided tests, this tests detect whether there is a monotonic trend across the different groups.
:::

## Jonckheere-Terpstra Test: `SAS`
...
## Jonckheere-Terpstra Test: `R`

An exact implementation of the Jonckheere-Terpstra test can be performed using the `JonckheereTerpstraTest()` function in the `DescTools` package.

::: callout-tip
Unlike the previous examples, the `JonckheereTerpstraTest()` function does not take a contingency table as input. It needs line-level input with a column for the `dependent` variable and the `independent` variable.

```{r}
#| eval: false

JonckheereTerpstraTest(dependent~independent,
                       data=example_data,
...
Please refer to `operate.csv` for the data.

```{r}
library(tidyverse)
operate <- read.csv("operate.csv") |> 
  uncount(weights=Count) |> 
  mutate(Treatment=factor(Treatment,levels=c("v+d","v+a","v+h","gre")),
         Severity=factor(Severity,levels=c("none","slight","moderate")))
glimpse(operate)
```
...
```{r}
JonckheereTerpstraTest(Severity~Treatment,data=operate,alternative="increasing",nperm=10000)
```
