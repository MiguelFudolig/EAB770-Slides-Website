...
::: callout-note
The variable that define the strata may be an explanatory variable, location, or classification variable. Each contingency table corresponds to one level of the explanatory variables (stratum)
:::
...
The Cochran-Mantel-Haenszel test assess the overall association of group and response after adjusting for the strata. If $n_{j11}$ is the $n_{11}$ value of the $j^{th}$ contingency table, the Cochran-Mantel-Haenszel statistic, $Q_{MH}$ can be written as:
...
Just like the previous chapter, we can use the `CMHtest()` function in the `vcdExtra` package in `R`.
...
library(vcdExtra)
CMHtest(contingency_table,overall=T,strata=strata_var)
```
...
1. Testing for an association between sex and disease status
