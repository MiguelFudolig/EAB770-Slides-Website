...
::: callout-note
The proportional odds assumption can be tested using the Brant test. The null hypothesis of the test is that the proportional odds assumption holds. For the proportional odds assumption to hold, we must fail to reject the null hypothesis.
:::
...
The `polr()` function in the `MASS` package in `R` performs the proportional odds model in `R`. Marginal/conditional means can still be calculated using `emmeans()` from the `emmeans` package.

::: callout-note
The `poTest()` function in `car` tests whether the proportional odds assumption holds. 
:::


::: callout-note
Because the `polr()` function uses a slightly different link between the cumulative logits, the interpretation for these numbers will be different from that of `SAS`. Calculating odds ratios will provide the odds ratio of recording higher level of outcomes between the two predictor levels.
:::
...
```{r}
library(tidyverse)
arthritis <- read.csv("arthritis_9.csv",stringsAsFactors = T)
glimpse(arthritis)
```

It is important to check if the factor levels follow the order we want.

```{r}
levels(arthritis$Improvement)
...
arthritis <- arthritis |>
  mutate(Improvement = factor(Improvement, levels = c("None","Some", "Marked"),ordered=T),
...
mod_po <- polr(Improvement~Treatment+Sex,data=arthritis,Hess = TRUE)
summary(mod_po)
...
```{r}
library(emmeans)
emm_cum <- emmeans(mod_po,~Treatment*Sex|cut,mode="cum.prob")
emm_cum


emm_prob <- emmeans(mod_po,~Treatment*Sex|Improvement,mode="prob")
emm_prob
emm_prob |> plot()

```

To calculate the odds ratio, we can use `contrast()` but we have to manually exponentiate the confidence limits.

```{r}
emm_treatment <- emmeans(mod_po,~Treatment)
contrast(emm_treatment,method="revpairwise",infer=T) |> 
  as.data.frame() |> 
  mutate(OR = exp(estimate),
         OR.LCL = exp(asymp.LCL),
         OR.UCL = exp(asymp.UCL))

emm_sex <- emmeans(mod_po,~Sex)
contrast(emm_sex,method="pairwise",infer=T) |> 
  as.data.frame() |> 
  mutate(OR = exp(estimate),
         OR.LCL = exp(asymp.LCL),
         OR.UCL = exp(asymp.UCL))
...
Females have 3.74 (95% CI: 1.33, 10.55) times the odds of marked improvement compared to males.

The odds of improvement after undergoing active treatment is 6.03 (95% CI: 2.39, 15.2) times that of the placebo group.
:::
...
A `csv` file is uploaded on Canvas as `air_pollution.csv`.

```{r}
library(tidyverse) 
air_pollution <- read.csv("air_pollution.csv",stringsAsFactors = TRUE)
air_pollution <- air_pollution |> 
  mutate(Level = factor(Level, ordered=T))

glimpse(air_pollution)
```
...
library(car)
options(contrasts = c("contr.treatment", "contr.poly"))
mod_po <- polr(Level~Air + Exposure + Smoking, data=air_pollution,Hess=TRUE)
...
```{r}
library(emmeans)
emm_cum <- emmeans(mod_po,~Air+Exposure+Smoking|cut,mode="cum.prob")
emm_cum


emm_prob <- emmeans(mod_po,~Air+Exposure+Smoking|Level,mode="prob")
emm_prob
...
```{r}
emm_Air <- emmeans(mod_po,~Air)
contrast(emm_Air,method="revpairwise",infer=T) |> 
  as.data.frame() |> 
  mutate(OR = exp(estimate),
         OR.LCL = exp(asymp.LCL),
         OR.UCL = exp(asymp.UCL))

emm_Ex <- emmeans(mod_po,~Exposure)
contrast(emm_Ex,method="revpairwise",infer=T) |> 
  as.data.frame() |> 
  mutate(OR = exp(estimate),
         OR.LCL = exp(asymp.LCL),
         OR.UCL = exp(asymp.UCL))

emm_Smoking <- emmeans(mod_po,~Smoking)
contrast(emm_Smoking,method="pairwise",infer=T,adjust="tukey") |> 
  as.data.frame() |> 
  mutate(OR = exp(estimate),
         OR.LCL = exp(asymp.LCL),
         OR.UCL = exp(asymp.UCL))
...
There is no evidence of an air pollution effect ($\chi^2 = 0.18, p=0.68$). There is evidence of a strong association between exposure ($\chi^2 = 82.1, p=<0.001$) and smoking history ($\chi^2 = 209.9, p<0.001$).

The odds of being assigned to Response Level III and lower for current smokers is 0.23 times the odds for ex-smokers. The odds of being assigned to Response Level 4 for current smokers is 4.27 times the odds for ex-smokers.
...
## Partial Proportional Odds Model in `R`

If the score test for the proportional odds assumption led to the rejection of the null hypothesis at the 0.10 level, we might need to use a partial proportional odds model.

::: callout-note
The `clm()` package in the `ordinal` package in `R` can be used to perform partial proportional odds model. Suppose we have two predictors, `x1` and `x2`, one of which did not satisfy the proportional odds model.
...
It can also be used to estimate proportional odds models. 
:::
...
```{r}
library(ordinal)
mod1 <-clm(Improvement~1,nominal = ~ Sex + Treatment,data=arthritis)
mod3 <- clm(Improvement~ Sex,nominal=~ Treatment,data=arthritis)
mod4 <- clm(Improvement~ Treatment,nominal=~ Sex,data=arthritis)
...
There is insufficient evidence to claim that the $\beta$ for sex ($\chi^2 = 1.35, p=0.25$) and treatment ($\chi^2 = 0.03, p=0.86$) are different across the different outcome levels.
